...
open(F, "/opt/padavan-ng/trunk/user/tor/tor-0.4.X.X/src/app/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/opt/padavan-ng/trunk/user/tor/tor-0.4.X.X/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/opt/padavan-ng/trunk/user/tor/tor-0.4.X.X/doc/man/tor.1.txt") or die;
while (<F>) {
